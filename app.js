...
const AdvancedStatsModule = (() => {
  const STORAGE_KEY = 'mimic_advanced_stats';
  ...
  function init() {
    try {
      ...
      stats = saved ? JSON.parse(saved) : createEmptyStats();
    } catch (e) {
  ...
  function createEmptyStats() {
    ...
    EMOTION_KEYS.forEach(key => {
      empty[key] = { attempts: 0, correct: 0, totalTime: 0 };
    });
    return empty;
  }
  ...
  function recordSuccess(emotionKey) {
    if (!stats[emotionKey]) stats[emotionKey] = { attempts: 0, correct: 0, totalTime: 0 };
    stats[emotionKey].attempts++;
    stats[emotionKey].correct++;
    
    // Если время старта не было установлено (равно 0), берем текущий момент
    const validStartTime = emotionStartTime > 0 ? emotionStartTime : Date.now();
    let timeTaken = (Date.now() - validStartTime) / 1000;
    
    //  Игнорируем аномально большие значения (больше 30 секунд - явный баг)
    if (timeTaken > 0 && timeTaken <= 30) {
      stats[emotionKey].totalTime += timeTaken;
    }
    
    save();
    emotionStartTime = Date.now(); // Сброс для следующей попытки
  }
  ...
  function recordFailure(emotionKey) {
    ...
    stats[emotionKey].attempts++;
    save();
    // При ошибке сбрасываем таймер, чтобы следующая попытка считалась с нуля
    emotionStartTime = Date.now();
  }

  function getAggregatedData() {
    return EMOTION_KEYS.map(key => {
      const d = stats[key] || { attempts: 0, correct: 0, totalTime: 0 };
  ...
  function closeModal() {
    ...
    if (modal) {
      ...
      setTimeout(() => {
        modal.hidden = true;
      }, 300); // Ждём 300мс (время анимации в CSS)
    }
...
function renderRadarChart() {
  ...
  // Берём переводы из текущего языка
  const t = translations[currentLang] || {};
  const emotionNames = {
  ...
  // Определяем текущую тему для адаптивных цветов
  const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
  const gridColor = isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)';
  const textColor = isDark ? 'rgba(255,255,255,0.9)' : '#212529';
  const legendColor = isDark ? 'rgba(255,255,255,0.8)' : '#212529';

  // Переводы для легенды
  const accuracyLabel = t.legendAccuracy || 'Точность (%)';
  const speedLabel = t.legendSpeed || 'Скорость реакции';
...
function renderStatsTable() {
  ...
  tbody.innerHTML = data.map(d => {
    ...
    return `<tr>
      <td class="emotion-name">${emotionNames[d.emotion] || d.emotion}</td>
      <td>${d.attempts}</td>
      <td>${d.correct}</td>
  ...
  function renderStatsTable() {
    ...
    //  Динамический перевод названий эмоций + эмодзи
    const t = translations[currentLang];
    const names = {
      happy: `${t.emotionHappy || 'Happy'} 😊`,
      sad: `${t.emotionSad || 'Sad'} 😢`,
      angry: `${t.emotionAngry || 'Angry'} 😠`,
      surprised: `${t.emotionSurprised || 'Surprised'} 😮`,
      fearful: `${t.emotionFearful || 'Fear'} 😨`,
      disgusted: `${t.emotionDisgusted || 'Disgust'} 🤢`
    };
...
function onThemeChange() {
  const modal = document.getElementById('advanced-stats-modal');
  if (modal && !modal.hidden && modal.classList.contains('active')) {
    renderRadarChart();
  }
}

// Наблюдаем за изменением атрибута data-theme
...
// Функция для обновления при смене языка (вызывай из switchLanguage)
